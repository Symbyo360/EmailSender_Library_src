...
////////////////////////////////////////////////////////////////////////////////
// ssl_st/TLS support added by John Tang by making use of OpenSSL: http://www.openssl.org/ 
...
{
	{command_INIT,          0,     5*60,  220, MailError::SERVER_NOT_RESPONDING},
	{command_EHLO,          5*60,  5*60,  250, MailError::COMMAND_EHLO},
	{command_AUTHPLAIN,     5*60,  5*60,  334, MailError::COMMAND_AUTH_PLAIN},
	{command_AUTHLOGIN,     5*60,  5*60,  334, MailError::COMMAND_AUTH_LOGIN},
	{command_AUTHCRAMMD5,   5*60,  5*60,  334, MailError::COMMAND_AUTH_CRAMMD5},
	{command_AUTHDIGESTMD5, 5*60,  5*60,  334, MailError::COMMAND_AUTH_DIGESTMD5},
	{command_DIGESTMD5,     5*60,  5*60,  335, MailError::COMMAND_DIGESTMD5},
	{command_USER,          5*60,  5*60,  334, MailError::UNDEF_XYZ_RESPONSE},
	{command_PASSWORD,      5*60,  5*60,  235, MailError::BAD_LOGIN_PASS},
	{command_MAILFROM,      5*60,  5*60,  250, MailError::COMMAND_MAIL_FROM},
	{command_RCPTTO,        5*60,  5*60,  250, MailError::COMMAND_RCPT_TO},
	{command_DATA,          5*60,  2*60,  354, MailError::COMMAND_DATA},
	{command_DATABLOCK,     3*60,  0,     0,   MailError::COMMAND_DATABLOCK},	// Here the valid_reply_code is set to zero because there are no replies when sending data blocks
	{command_DATAEND,       3*60,  10*60, 250, MailError::MSG_BODY_ERROR},
	{command_QUIT,          5*60,  5*60,  221, MailError::COMMAND_QUIT},
	{command_STARTTLS,      5*60,  5*60,  220, MailError::COMMAND_EHLO_STARTTLS}
};
...
EmailSender::EmailSender()
{
	...
	if (WSAStartup(wVer,&wsaData) != NO_ERROR)
		throw MailError(MailError::WSA_STARTUP);
	if (LOBYTE( wsaData.wVersion ) != 2 || HIBYTE( wsaData.wVersion ) != 2 ) 
	{
		WSACleanup();
		throw MailError(MailError::WSA_VER);
	}
#endif
	
	if((RecvBuf = new char[BUFFER_SIZE]) == NULL)
		throw MailError(MailError::LACK_OF_MEMORY);
...
//							JP 2010-07-07
////////////////////////////////////////////////////////////////////////////////
void EmailSender::AddAttachment(const char *Path)
{
	assert(Path);
...
//							JP 2010-07-07
////////////////////////////////////////////////////////////////////////////////
void EmailSender::AddRecipient(const char *email, const char *name)
{	
	if(!email)
		throw MailError(MailError::UNDEF_RECIPIENT_MAIL);
...
//							JP 2010-07-07
////////////////////////////////////////////////////////////////////////////////
void EmailSender::AddCCRecipient(const char *email, const char *name)
{	
	if(!email)
		throw MailError(MailError::UNDEF_RECIPIENT_MAIL);
...
//							JP 2010-07-07
////////////////////////////////////////////////////////////////////////////////
void EmailSender::AddBCCRecipient(const char *email, const char *name)
{	
	if(!email)
		throw MailError(MailError::UNDEF_RECIPIENT_MAIL);
...
//							JP 2010-07-07
////////////////////////////////////////////////////////////////////////////////
void EmailSender::AddMsgLine(const char* Text)
{
...
//							JP 2010-07-07
////////////////////////////////////////////////////////////////////////////////
void EmailSender::DelMsgLine(unsigned int Line)
{
	if(Line > MsgBody.size())
		throw MailError(MailError::OUT_OF_MSG_RANGE);
...
//							JP 2010-07-07
////////////////////////////////////////////////////////////////////////////////
void EmailSender::DelRecipients()
{
	Recipients.clear();
}
...
//							JP 2010-07-07
////////////////////////////////////////////////////////////////////////////////
void EmailSender::DelBCCRecipients()
{
	BCCRecipients.clear();
}
...
//							JP 2010-07-07
////////////////////////////////////////////////////////////////////////////////
void EmailSender::DelCCRecipients()
{
	CCRecipients.clear();
}
...
//							JP 2010-07-07
////////////////////////////////////////////////////////////////////////////////
void EmailSender::DelAttachments()
{
	Attachments.clear();
}
...
//							JP 2010-07-07
////////////////////////////////////////////////////////////////////////////////
void EmailSender::ModMsgLine(unsigned int Line,const char* Text)
{
	if(Text)
	{
		if(Line > MsgBody.size())
...
void EmailSender::Send()
{
	...
	if(hSocket==NULL)
	{
		if(!ConnectRemoteServer(m_sSMTPSrvName.c_str(), m_iSMTPSrvPort))
			throw MailError(MailError::WSA_INVALID_SOCKET);
	}
	...
	try{
		...
		if(!m_sMailFrom.size())
			throw MailError(MailError::UNDEF_MAIL_FROM);
		...
		if(!(rcpt_count = Recipients.size()))
			throw MailError(MailError::UNDEF_RECIPIENTS);
		...
		for(FileId=0;FileId<Attachments.size();FileId++)
		{
			...
			if(hFile == NULL)
				throw MailError(MailError::FILE_NOT_EXIST);
			...
			if(TotalSize/1024 > MSG_SIZE_IN_MB*1024)
				throw MailError(MailError::MSG_TOO_BIG);
			else
			{
				...
				for(i=0;i<FileSize/54+1;i++)
				{
					...
					MsgPart ? strcat(SendBuf,StringUtility::encodeToBase64String(reinterpret_cast<const unsigned char*>(FileBuf),res).c_str())
							  : strcpy(SendBuf,StringUtility::encodeToBase64String(reinterpret_cast<const unsigned char*>(FileBuf),res).c_str());
					strcat(SendBuf,"\r\n");
					MsgPart += res + 2;
...
// AUTHOR/DATE: JP 2010-01-28
////////////////////////////////////////////////////////////////////////////////
bool EmailSender::ConnectRemoteServer(const char *szServer, const unsigned short nPort_)
{
	...
	try
	{
		...
		if((sockAddr.sin_addr.s_addr = inet_addr(szServer)) == INADDR_NONE)
		{
			...
			else
			{
				...
				closesocket(hSocket);
#endif
				throw MailError(MailError::WSA_GETHOSTBY_NAME_ADDR);
			}				
		}
...
#endif
		{
			...
			closesocket(hSocket);
#endif
			throw MailError(MailError::WSA_IOCTLSOCKET);
		}
		...
		if(connect(hSocket,(LPSOCKADDR)&sockAddr,sizeof(sockAddr)) == SOCKET_ERROR)
		{
...
#endif
			{
				...
				closesocket(hSocket);
#endif
				throw MailError(MailError::WSA_CONNECT);
			}
		}
		else
			return true;
		...
		while(true)
		{
			...
			if((res = select(hSocket+1,NULL,&fdwrite,&fdexcept,&timeout)) == SOCKET_ERROR)
			{
				...
				closesocket(hSocket);
#endif
				throw MailError(MailError::WSA_SELECT);
			}
			...
			if(!res)
			{
				...
				closesocket(hSocket);
#endif
				throw MailError(MailError::SELECT_TIMEOUT);
			}
		...
		{
			if(!m_sLogin.size())
				throw MailError(MailError::UNDEF_LOGIN);

			if(!m_sPassword.size())
				throw MailError(MailError::UNDEF_PASSWORD);
			...
			if(IsKeywordSupported(RecvBuf, "LOGIN") == true)
			{
				...
				// send login:
				std::string encoded_login = StringUtility::encodeToBase64String(reinterpret_cast<const unsigned char*>(m_sLogin.c_str()),m_sLogin.size());
				...
				// send password:
				std::string encoded_password = StringUtility::encodeToBase64String(reinterpret_cast<const unsigned char*>(m_sPassword.c_str()),m_sPassword.size());
			...
			else if(IsKeywordSupported(RecvBuf, "PLAIN") == true)
			{
				...
				std::string encoded_login = StringUtility::encodeToBase64String(ustrLogin, strlen(SendBuf));
				delete[] ustrLogin;
			...
			else if(IsKeywordSupported(RecvBuf, "CRAM-MD5") == true)
			{
				...
				std::string decoded_challenge = StringUtility::decodeFromBase64String(encoded_challenge);
				...
				if(!ustrChallenge || !ustrPassword)
					throw MailError(MailError::BAD_LOGIN_PASSWORD);
				...
				encoded_challenge = StringUtility::encodeToBase64String(reinterpret_cast<const unsigned char*>(decoded_challenge.c_str()),decoded_challenge.size());
			...
			else if(IsKeywordSupported(RecvBuf, "DIGEST-MD5") == true)
			{
				...
				if(find<0)
					throw MailError(MailError::BAD_DIGEST_RESPONSE);
				...
				if(!getpeername(hSocket, (struct sockaddr*)&addr, &len))
					throw MailError(MailError::BAD_SERVER_NAME);
				...
				encoded_challenge = StringUtility::encodeToBase64String(ustrDigest, strlen(SendBuf));
				delete[] ustrDigest;
				sprintf(SendBuf, "%s\r\n", encoded_challenge.c_str());
				pEntry = FindCommandEntry(command_DIGESTMD5);
				SendData(pEntry);
			...
			else throw MailError(MailError::LOGIN_NOT_SUPPORTED);
		}
	}
	catch(const MailError&)
	{
...
////////////////////////////////////////////////////////////////////////////////
void EmailSender::DisconnectRemoteServer()
{
...
//							JP 2010-07-07
////////////////////////////////////////////////////////////////////////////////
void EmailSender::FormatHeader(char* header)
{
	...
	if(time(&rawtime) > 0)
		...
		throw MailError(MailError::TIME_ERROR);
...
////////////////////////////////////////////////////////////////////////////////
void EmailSender::ReceiveData(Command_Entry* pEntry)
{
	if(m_ssl != NULL)
	{
	...
	if(RecvBuf == NULL)
		throw MailError(MailError::RECVBUF_IS_EMPTY);
	...
	if(!res)
	{
		//timeout
		FD_CLR(hSocket,&fdread);
		throw MailError(MailError::SERVER_NOT_RESPONDING);
	}
	...
	if(FD_ISSET(hSocket,&fdread))
	{
		...
		{
			FD_CLR(hSocket,&fdread);
			throw MailError(MailError::WSA_RECV);
		}
	}
	...
	if(res == 0)
	{
		throw MailError(MailError::CONNECTION_CLOSED);
	}
}
...
// AUTHOR/DATE: JP 2010-01-28
////////////////////////////////////////////////////////////////////////////////
void EmailSender::SendData(Command_Entry* pEntry)
{
	if(m_ssl != NULL)
	{
	...
	if(SendBuf == NULL)
		throw MailError(MailError::SENDBUF_IS_EMPTY);
	...
	while(nLeft > 0)
	{
		...
		if(res && FD_ISSET(hSocket,&fdwrite))
		{
			...
			{
				FD_CLR(hSocket,&fdwrite);
				throw MailError(MailError::WSA_SEND);
			}
			nLeft -= res;
			idx += res;
		}
...
// AUTHOR/DATE: JP 2010-01-28
////////////////////////////////////////////////////////////////////////////////
const char* EmailSender::GetLocalHostName()
{
	char* str = NULL;
	...
	{
		delete[] str;
		throw MailError(MailError::WSA_HOSTNAME);
	}
...
// AUTHOR/DATE: JP 2010-01-28
////////////////////////////////////////////////////////////////////////////////
unsigned int EmailSender::GetRecipientCount() const
{
...
// AUTHOR/DATE: JP 2010-01-28
////////////////////////////////////////////////////////////////////////////////
unsigned int EmailSender::GetBCCRecipientCount() const
{
...
// AUTHOR/DATE: JP 2010-01-28
////////////////////////////////////////////////////////////////////////////////
unsigned int EmailSender::GetCCRecipientCount() const
{
...
// AUTHOR/DATE: JP 2010-01-28
////////////////////////////////////////////////////////////////////////////////
const char* EmailSender::GetReplyTo() const
{
...
// AUTHOR/DATE: JP 2010-01-28
////////////////////////////////////////////////////////////////////////////////
const char* EmailSender::GetMailFrom() const
{
...
// AUTHOR/DATE: JP 2010-01-28
////////////////////////////////////////////////////////////////////////////////
const char* EmailSender::GetSenderName() const
{
...
// AUTHOR/DATE: JP 2010-01-28
////////////////////////////////////////////////////////////////////////////////
const char* EmailSender::GetSubject() const
{
...
// AUTHOR/DATE: JP 2010-01-28
////////////////////////////////////////////////////////////////////////////////
const char* EmailSender::GetXMailer() const
{
...
// AUTHOR/DATE: JP 2010-01-28
////////////////////////////////////////////////////////////////////////////////
CSmptXPriority EmailSender::GetXPriority() const
{
	return m_iXPriority;
}

const char* EmailSender::GetMsgLineText(unsigned int Line) const
{
	if(Line > MsgBody.size())
		throw MailError(MailError::OUT_OF_MSG_RANGE);
...
unsigned int EmailSender::GetMsgLines() const
{
	return MsgBody.size();
}
...
// AUTHOR/DATE: JP 2010-01-28
////////////////////////////////////////////////////////////////////////////////
void EmailSender::SetXPriority(CSmptXPriority priority)
{
...
//							JP 2010-07-08
////////////////////////////////////////////////////////////////////////////////
void EmailSender::SetReplyTo(const char *ReplyTo)
{
	m_sReplyTo.erase();
...
//							JP 2010-07-08
////////////////////////////////////////////////////////////////////////////////
void EmailSender::SetSenderMail(const char *EMail)
{
	m_sMailFrom.erase();
...
//							JP 2010-07-08
////////////////////////////////////////////////////////////////////////////////
void EmailSender::SetSenderName(const char *Name)
{
	m_sNameFrom.erase();
...
//							JP 2010-07-08
////////////////////////////////////////////////////////////////////////////////
void EmailSender::SetSubject(const char *Subject)
{
	m_sSubject.erase();
...
//							JP 2010-07-08
////////////////////////////////////////////////////////////////////////////////
void EmailSender::SetXMailer(const char *XMailer)
{
	m_sXMailer.erase();
...
//							JP 2010-07-08
////////////////////////////////////////////////////////////////////////////////
void EmailSender::SetLogin(const char *Login)
{
	m_sLogin.erase();
...
//							JP 2010-07-08
////////////////////////////////////////////////////////////////////////////////
void EmailSender::SetPassword(const char *Password)
{
	m_sPassword.erase();
...
//							JO 2010-0708
////////////////////////////////////////////////////////////////////////////////
void EmailSender::SetSMTPServer(const char* SrvName,const unsigned short SrvPort)
{
...
void EmailSender::StartTls()
{
	if(IsKeywordSupported(RecvBuf, "STARTTLS") == false)
	{
		throw MailError(MailError::STARTTLS_NOT_SUPPORTED);
	}
...
void EmailSender::ReceiveData_SSL(ssl_st* ssl, Command_Entry* pEntry)
{
	int res = 0;
	int offset = 0;
	fd_set fdread;
		...
		if(FD_ISSET(hSocket,&fdread) || (read_blocked_on_write && FD_ISSET(hSocket,&fdwrite)) )
		{
			while(1)
			{
				...
				else
				{
					FD_ZERO(&fdread);
					FD_ZERO(&fdwrite);
					throw MailError(MailError::SSL_PROBLEM);
				}
			}
		}
	}
...
void EmailSender::ReceiveResponse(Command_Entry* pEntry)
{
	std::string line;
	int reply_code = 0;
	bool bFinish = false;
	...
	{
		throw MailError(pEntry->error);
	}
}

void EmailSender::SendData_SSL(ssl_st* ssl, Command_Entry* pEntry)
{
...
void EmailSender::OpenSSLConnect()
{
	if(m_ctx == NULL)
		throw MailError(MailError::SSL_PROBLEM);
	m_ssl = SSL_new (m_ctx);   
...
void EmailSender::CleanupOpenSSL()
{
	if(m_ssl != NULL)
	{
		SSL_shutdown (m_ssl);  /* send ssl_st/TLS close_notify */
		SSL_free (m_ssl);
		m_ssl = NULL;
	}
	if(m_ctx != NULL)
